...
function renderizarAula(data) {
    ...
    document.getElementById("aula-trilha").innerText =
        data.trilha?.nome || "Trilha";

    document.getElementById("aula-nome").innerText =
        curso.nome || "Aula";

    document.getElementById("aula-descricao").innerText =
        curso.descricao || "Sem descrição disponível.";

    document.getElementById("aula-video").src =
        converterYoutubeEmbed(curso.aula_video || "");

    document.getElementById("aula-texto").innerHTML =
        curso.aula_texto || "<p>Nenhum texto cadastrado para esta aula.</p>";

    document.getElementById("btn-voltar").onclick = () => {

        window.location.href =
            `detalhes_da_trilha.html?id=${idTrilhaAtual}`;
...
function mostrarVideo() {
    ...
    document.getElementById("area-video")
        .classList.add("active");

    document.getElementById("area-texto")
        .classList.remove("active");

    document.getElementById("btn-video")
        .classList.add("active");

    document.getElementById("btn-texto")
        .classList.remove("active");
...
function concluirAula() {
    ...
    .then(data => {
        ...
        if (data.retornar_trilha) {
            ...
            window.location.href =
                `detalhes_da_trilha.html?id=${data.id_trilha}`;
        ...
        if (
            data.proximo_curso &&
            data.proximo_curso.id_curso
        ) {

            window.location.href =
                `aula.html?id=${data.proximo_curso.id_curso}`;
...
function exibirConclusaoTrilha(data) {
    ...
    document.getElementById("btn-voltar-certificado").onclick = () => {
        ...
        window.location.href =
            `detalhes_da_trilha.html?id=${data.certificado.id_trilha}`;
    ...
    */
    ...
    document.getElementById("btn-acessar-certificado").href =
        `../backend/certificado_conclusao.php?id_trilha=${data.certificado.id_trilha}`;
